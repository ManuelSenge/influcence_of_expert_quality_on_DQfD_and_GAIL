...
class DQfD:
    def __init__(self, env, config, demo_transitions=None):
        config_sess = tf.compat.v1.ConfigProto(
            intra_op_parallelism_threads=1,  # Number of threads used for intra-op parallelism
            inter_op_parallelism_threads=1   # Number of threads used for inter-op parallelism
        )
        self.sess = tf.InteractiveSession(config=config_sess)
        self.config = config
        ...
        self.eval_input = tf.placeholder("float", [None, self.state_dim]) # self.state_dim
    ...
    def build_layers(self, state, c_names, units_1, units_2, w_i, b_i, reg=None):
        a_d = self.action_dim
        #a_d = self.state_dim # this is from me
        with tf.variable_scope('l1', reuse=tf.AUTO_REUSE):
            ...
            print('state', state.shape, 'w1', w1.shape, 'b1', b1.shape)
    ...
    def Q_select(self):
        with tf.variable_scope('select_net') as scope:
            ...
            reg = tf.keras.regularizers.l2(l2=0.2)
