...
def run_DQfD(index, env, use_wandb):
...
if __name__ == '__main__':
    ...
    if only_gather_data:
        ...
        exp_names = ['bad_data_under_100']#'mixed_good_400_500', 'middle_300_400', 'bad_data_under_100', 'bad_data_100_to_300'] # perfect_data_500
        env = gym.make(Config.ENV_NAME)
        for e in exp_names:
            if use_wandb:
                run = wandb.init(
                        name=f"demo_data_{exp_names}",
            ...
            get_demo_data(env, use_wandb, lower_goal_score=task2range[e][0], higher_goal_score=task2range[e][0], demo_data_path=path_data+'demo.p')
            wandb.close()
    else:
        create_dataset = True
        experiment_name = 'default'
        path_data = f"/Users/manuelsenge/Documents/TUM/Semester 4/Robotic_Seminar/workspace/DQfD_new/DQfD/data/{experiment_name}"
        if use_wandb:
            run = wandb.init(
                    name="full_policy_DDQN",
                    project="robotic_seminar",
                    group="standard_DDQN",
                    entity="manuelsenge",
                    sync_tensorboard=True,  # auto-upload sb3's tensorboard metrics
